...
onMounted(async () => {
  try {
    ...
    // non bloquant
        ...
        ouvrez les outils développeur (F12), onglet <em>Réseau</em>, rechargez la page,
        puis exportez les requêtes au format <strong>.har</strong>.
        Importez le fichier ci-dessous pour récupérer vos produits favoris.
      </p>

      <!-- Étape 1 : Upload HAR -->
      <div v-if="etape === 'upload'" class="flex flex-col items-start gap-3">
        <label
          class="flex flex-col items-center justify-center w-full border-2 border-dashed border-gray-300 rounded-xl p-8 cursor-pointer hover:border-emerald-400 transition-colors"
          @dragover.prevent
          @drop.prevent="onDrop"
      ...
      <!-- Chargement parsing -->
      <div v-if="etape === 'parsing'" class="text-center py-8 text-gray-400">
        <div class="inline-block animate-spin text-3xl mb-2">⚙️</div>
        <p>Analyse du fichier HAR en cours…</p>
      </div>
      ...
      <div v-if="etape === 'selection'" class="space-y-4">
        <div class="flex items-center justify-between gap-3 flex-wrap">
          <span class="text-sm text-gray-600">
            <strong>{{ produits.length }}</strong> produits trouvés —
            <strong>{{ selectionnes }}</strong> sélectionné(s)
          </span>
          <div class="flex gap-2 flex-wrap">
            <input
              v-model="recherche"
            ...
            />
            <button @click="toutCocher(true)" class="text-xs px-3 py-1.5 rounded-lg border border-gray-300 hover:bg-gray-50">
              Tout cocher
            </button>
            <button @click="toutCocher(false)" class="text-xs px-3 py-1.5 rounded-lg border border-gray-300 hover:bg-gray-50">
              Tout décocher
            </button>
            <button
              @click="reinitialiser"
        ...
        <div class="overflow-x-auto rounded-xl border">
          <table class="min-w-full text-sm">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-3 py-2 w-8"><input type="checkbox" @change="toutCocher(($event.target as HTMLInputElement).checked)" :checked="selectionnes === produitsFiltres.length && selectionnes > 0" /></th>
                <th class="px-3 py-2 w-16 text-left text-xs text-gray-500 uppercase">Image</th>
                <th class="px-3 py-2 text-left text-xs text-gray-500 uppercase">Nom (modifiable)</th>
                <th class="px-3 py-2 w-32 text-center text-xs text-gray-500 uppercase hidden sm:table-cell">Achat systématique</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <tr v-for="p in produitsFiltres" :key="p.external_id" :class="p.selected ? '' : 'opacity-40'">
      ...
      <div v-if="etape === 'resultat'" class="space-y-4">
        <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-5">
          <p class="text-emerald-800 font-semibold text-base">✅ Import terminé !</p>
          <ul class="mt-2 text-sm text-emerald-700 space-y-1">
            <li>🆕 <strong>{{ resultat.created }}</strong> ingrédient(s) créé(s)</li>
            <li>⏭️ <strong>{{ resultat.skipped }}</strong> déjà existant(s) (ignoré(s))</li>
          </ul>
          <div v-if="resultat.created_names.length" class="mt-3 text-xs text-emerald-600 max-h-32 overflow-y-auto">
            <p class="font-medium mb-1">Créés :</p>
            <p v-for="n in resultat.created_names" :key="n" class="leading-tight">{{ n }}</p>
          </div>
        </div>
        <button @click="reinitialiser" class="text-sm text-gray-600 underline hover:text-gray-900">
...
const etape = ref<'upload' | 'parsing' | 'selection' | 'resultat'>('upload')
const produits = ref<Produit[]>([])
const recherche = ref('')
const erreur = ref('')
const importing = ref(false)
...
async function analyserFichier(file: File) {
  ...
  try {
    ...
    const data = await res.json() as { products: Omit<Produit, 'selected' | 'achat_systematique'>[], count: number }
    if (data.count === 0) {
      erreur.value = 'Aucun produit Coursesu trouvé dans ce fichier HAR. Vérifiez que le HAR contient bien les pages "mes-listes?isPref=true".'
      etape.value = 'upload'
      return
    }
    produits.value = data.products.map(p => ({ ...p, achat_systematique: false, selected: true }))
...
async function importer() {
  ...
  const items = produits.value
    .filter(p => p.selected && p.name.trim())
    .map(p => ({ name: p.name.trim(), image_url: p.image_url, achat_systematique: p.achat_systematique }))

  try {
    const res = await fetch(`${config.public.apiBase}/tools/import-ingredients/`, {
      method: 'POST',
    ...
    const data = await res.json() as { created: number, skipped: number, created_names: string[] }
    resultat.value = data
    etape.value = 'resultat'
  } catch (e: unknown) {
    erreur.value = (e as Error).message
